...
async function run() {
    try {
        ...
        const sheetsToProcess = wb.SheetNames.filter(name => !['نموذج (3)', 'نموذج (2)', 'الرئيسيه', 'حساب التحالف', 'خزنه احمد سامح', 'مدفوعات التحالف'].includes(name));
        
        console.log(`Found ${sheetsToProcess.length} valid sheets to process.`);
        ...
        for (const sheetName of sheetsToProcess) {
            ...
            let agentRes = await pool.query(`SELECT id FROM agents WHERE name = $1 LIMIT 1`, [sheetName]);
            let agentId;
            if (agentRes.rows.length === 0) {
                agentId = uuidv4();
                await pool.query(`INSERT INTO agents (id, name, type, source_sheet_name, currency, created_at, updated_at) VALUES ($1, $2, 'B2B', $3, 'EGP', NOW(), NOW())`, [agentId, sheetName, sheetName]);
                console.log(`  -> Created missing agent: ${sheetName}`);
            } else {
                agentId = agentRes.rows[0].id;
            }

            // 2. Wipe existing data for this agent so we don't duplicate
            ...
            // Very careful matching of headers
        ...
        console.log(`Success! Inserted ${totalPassengers} passengers and ${totalPayments} payments for ${sheetsToProcess.length} agents.`);
    } catch(e) {
