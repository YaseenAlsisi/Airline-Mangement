...
function classifyRow(row) {
    ...
    let isRowEmpty = true;
    for (let i = 0; i < 25; i++) { // increased range just in case
    ...
    for (let i = 16; i <= 21; i++) { // increased range
...
async function run() {
    ...
    try {
        ...
        console.log('Wiping old transactions...');
        ...
        for (const sheetName of wb.SheetNames) {
            ...
            for (let r = 1; r < data.length; r++) {
                ...
                txBatch.push([
                    ...
                    debitUsd, creditUsd, debitEgp, creditEgp, truncate(row[0], 255) // raw_column_a
                ]);
        ...
        console.log(`Success! Total Agents Processed: ${totalAgents}, Tx: ${totalTx}`);
    } catch (e) {
